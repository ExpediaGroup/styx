...
class ReactorHostHttpClientTest : StringSpec() {
    private val context: HttpInterceptor.Context = mockk(relaxed = true)
    ...
    init {
        ...
        "doOnResponse should be invoked if it is not null" {
            val mockResponse =
                MockResponse()
                    .setResponseCode(200)
                    .addHeader("X-Id", "123")
            ...
            val doOnResponse: (HttpClientResponse, HttpInterceptor.Context?) -> Unit = mockk(relaxed = true)
            ...
            val reactorHostClient =
                ReactorHostHttpClient.create(
                    ...
                    metrics = metrics,
                    eventLoopGroup = LOOP_RESOURCES,
                    doOnResponse = doOnResponse,
                )

            Mono.from(reactorHostClient.sendRequest(mockRequest, context)).block()

            verify(exactly = 1) {
                doOnResponse.invoke(ofType(HttpClientResponse::class), ofType(HttpInterceptor.Context::class))
            }
        }
