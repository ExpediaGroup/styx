...
public class RequestStatsCollector implements RequestProgressListener {
     ...
     * @param tags tags to attach to all metrics generated by this collector
     */
    public RequestStatsCollector(MeterRegistry registry, Iterable<Tag> tags) {
        this(registry, tags, NanoClock.SYSTEM_CLOCK);
    }

    RequestStatsCollector(MeterRegistry registry, Iterable<Tag> tags, NanoClock nanoClock) {

        this.registry = registry;
        this.tags = Tags.of(tags);

        this.nanoClock = nanoClock;
        this.outstandingRequests = registry.gauge(REQUEST_OUTSTANDING, tags, new AtomicLong());
        this.latencyTimer = registry.timer(REQUEST_LATENCY, tags);
        this.requestsIncoming = registry.counter(REQUEST_RECEIVED, tags);
        this.responsesSent = registry.counter(RESPONSE_SENT, tags);
    }
    ...
    private void updateResponseStatusCounter(int code) {
        ...
        Tags statusTags = tags.and(STATUS_CLASS_TAG, statusCodeClass);

        if ("5xx".equals(statusCodeClass)) {
            statusTags = statusTags.and(STATUS_TAG, valueOf(code));
        }

        registry.counter(RESPONSE_STATUS, statusTags).increment();
    }
