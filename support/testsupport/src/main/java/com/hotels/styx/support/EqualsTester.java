...
public final class EqualsTester {
    ...
    private void testItems() {
        for (Object item : Iterables.concat(equalityGroups)) {
            ...
            assertThat(item + " must not be Object#equals to an arbitrary object of another class",
                    !item.equals(NotAnInstance.EQUAL_TO_NOTHING));
            assertThat(item + " must be Object#equals to itself", item, is(item));
            assertThat("the Object#hashCode of " + item + " must be consistent", item.hashCode(), is(item.hashCode()));
        }
    }
