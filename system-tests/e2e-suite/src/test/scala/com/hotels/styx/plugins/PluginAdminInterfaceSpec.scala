...
class PluginAdminInterfaceSpec extends FunSpec with StyxProxySpec with StyxClientSupplier {
  ...
  def anHttpRequest: LiveHttpRequest = {
    LiveHttpRequest.get(styxServer.routerURL("/pluginPipelineSpec/")).build()
  }

  private class PluginX extends PluginAdapter {
    override def intercept(request: LiveHttpRequest, chain: HttpInterceptor.Chain): Eventual[HttpResponse] = chain.proceed(request)
