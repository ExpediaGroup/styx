  ...
  describe("Routes correctly") {
    it("Routes to origin indicated by cookie.") {
      val request = get("/app/")
        ...
        .cookies(cookie("originRestrictionCookie", "h2"))
        .build()
    ...
    it("Routes to range of origins indicated by cookie.") {
      val request = get("/app/")
        .header(HOST, styxServer.proxyHost)
        .cookies(cookie("originRestrictionCookie", "h(2|3)"))
        .build()
    ...
    it("If nothing matches treat as no hosts available") {
      val request = get("/app/")
        .header(HOST, styxServer.proxyHost)
        .cookies(cookie("originRestrictionCookie", "(?!)"))
        .build()
    ...
    it("Routes to list of origins indicated by cookie.") {
      val request = get("/app/")
        .header(HOST, styxServer.proxyHost)
        .cookies(cookie("originRestrictionCookie", "h2,h[3-4]"))
        .build()
